...
from dante.exceptions import DependencyMissingError
...
async def randomchannel(
    tochat, channel, range1, range2, caption=None, client=dante_bot
):
...
async def ReTrieveFile(input_file_name):
    ...
    async with aiohttp.ClientSession() as ses:
        async with ses.post(
            ...
            name = check_filename("dante-rmbg.png")
...
async def _insta_login():
    if "insta_creds" in dante_bot._cache:
        return dante_bot._cache["insta_creds"]
    ...
    if username and password:
        ...
        try:
            ...
            dante_bot._cache.update({"insta_creds": cl})
...
async def get_insta_code(username, choice):
    ...
    async with asst.conversation(dante_bot.uid, timeout=60 * 2) as conv:
